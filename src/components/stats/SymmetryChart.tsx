...
export function SymmetryChart({ data }: SymmetryChartProps) {
  ...
  const maxAbsDiff = chartData.length > 0 ? Math.max(...chartData.map((item) => Math.abs(item.diffPercent ?? 0))) : 0;
    ...
    <div className="space-y-4">
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} layout="vertical" margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          />
          <Bar dataKey="diffPercent" name="Percent difference" radius={[4, 4, 4, 4]} isAnimationActive={false}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {data
          .slice()
          .sort((a, b) => Math.abs((b["Percent Diff"] ?? 0) * 100) - Math.abs((a["Percent Diff"] ?? 0) * 100))
          .map((item, idx) => {
            const diff01 = item["Percent Diff"] ?? 0;
            ...
            return (
              <div key={idx} className="p-4 rounded-lg border-2 bg-card" style={{ borderColor: color }}>
