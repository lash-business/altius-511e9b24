...
function buildSetLabels(exercise: WorkoutExercise): string[] {
  ...
  const base = exercise.repsSeconds === "seconds" ? duration ? `${duration} Seconds` : "Seconds" : duration ? `${duration} Reps` : "Reps";
  return Array.from({
    length: count
  }, () => base);
}
...
export function Workout() {
    ...
    const fetchWorkout = async () => {
      try {
        ...
        const {
          data: tests,
          error: testsError
        } = await supabase.from("tests").select("id, test_date").eq("user_id", user.id).order("test_date", {
          ascending: false
        }).limit(1);
        ...
        const {
          data: workouts,
          error: workoutsError
        } = await supabase.from("workouts").select("id, week, day, completed_at").eq("test_id", latestTest.id).order("week", {
          ascending: true
        }).order("day", {
          ascending: true
        });
        ...
        } = await supabase.from("user_exercises").select("id, exercise_id, order").eq("workout_id", nextWorkout.id).order("order", {
          ascending: true
        });
        ...
        const exerciseIds = Array.from(new Set(typedUserExercises.map(ue => ue.exercise_id).filter((id): id is string => !!id)));
        let exercisesById = new Map<string, {
          id: string;
          name: string;
          sets: number | null;
        ...
        if (exerciseIds.length > 0) {
          ...
          } = await supabase.from("exercises").select("id, name, sets, reps_seconds, duration, video_link, equipment, setup, queues").in("id", exerciseIds);
          if (exercisesError) throw exercisesError;
          exercisesById = new Map((exercises || []).map((ex: any) => [ex.id as string, {
            id: ex.id as string,
        ...
        let initialStep = 0;
        const firstIncompleteIndex = exercises.findIndex(ex => normalizedCompletion[ex.userExerciseId]?.some(v => !v));
  ...
  const activeExercise: WorkoutExercise | null = viewState.status === "ready" && !isReviewStep ? viewState.data.exercises[currentStepIndex] : null;
  const overallStatus = useMemo(() => {
    ...
    const perExerciseComplete = viewState.data.exercises.map(ex => (completion[ex.userExerciseId] ?? []).every(Boolean));
    return {
      perExerciseComplete,
  ...
  const handleSubmitWorkout = async () => {
    ...
    try {
      ...
      const completedUserExerciseIds = viewState.data.exercises.filter(ex => (completion[ex.userExerciseId] ?? []).every(Boolean)).map(ex => ex.userExerciseId);
      const updates: Promise<unknown>[] = [];
      if (completedUserExerciseIds.length > 0) {
        updates.push(supabase.from("user_exercises").update({
          completed_at: now
        }).in("id", completedUserExerciseIds));
      }
      updates.push(supabase.from("workouts").update({
        completed_at: now
  ...
  const renderLoading = () => <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-8">
  ...
  const renderVideo = () => {
    if (viewState.status !== "ready" || !activeExercise) {
      return <div className="flex h-48 w-full items-center justify-center rounded-2xl bg-muted">
          <span className="text-sm text-muted-foreground">Loading video…</span>
        </div>;
    }
    const embedUrl = getVimeoEmbedUrl(activeExercise.videoLink);
    if (!embedUrl) {
      return <div className="flex h-48 w-full flex-col items-center justify-center gap-2 rounded-2xl bg-muted">
    ...
    return <div className="relative h-48 w-full overflow-hidden rounded-2xl bg-muted">
        <iframe src={embedUrl} title={activeExercise.name} className="h-full w-full" allow="autoplay; fullscreen; picture-in-picture" />
      </div>;
  };
  const renderProgressBar = () => {
    ...
    return <div className="flex gap-2">
        ...
        return <button key={ex.userExerciseId} type="button" onClick={() => handleJumpToExercise(index)} className={cn("flex-1 rounded-full px-2 py-1 text-xs font-medium text-background", "transition-colors", isComplete ? "bg-foreground" : isActive ? "bg-foreground/80" : "bg-muted text-muted-foreground")}>
  ...
  const renderDescriptionPreview = () => {
    ...
    return <button type="button" onClick={() => setIsDescriptionOpen(true)} className="flex w-full items-start gap-3 rounded-xl border bg-muted/40 px-4 py-3 text-left">
  ...
  const renderSetsTable = () => {
    ...
    return <div className="mt-4 space-y-3 rounded-2xl bg-card p-4 shadow-sm">
        ...
        {labels.map((label, index) => <div key={index} className="grid grid-cols-[auto,1fr,auto] items-center gap-3 rounded-xl bg-muted/40 px-3 py-2">
            <span className="text-sm font-semibold">{index + 1}</span>
            <span className="text-sm">{label}</span>
            <div className="flex justify-end">
              <Switch checked={!!exerciseCompletion[index]} onCheckedChange={() => handleToggleSet(activeExercise.userExerciseId, index)} />
            </div>
          </div>)}
      </div>;
  };
  const renderDescriptionSheet = () => {
    ...
    return <Sheet open={isDescriptionOpen} onOpenChange={setIsDescriptionOpen}>
    ...
    return <div className="space-y-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-72">
            {viewState.data.exercises.map((exercise, index) => <DropdownMenuItem key={exercise.userExerciseId} onClick={() => handleJumpToExercise(index)} className={cn("flex items-center justify-between gap-3", index === currentStepIndex && "bg-accent")}>
  ...
  const renderReviewPage = () => {
    ...
    return <div className="mt-4 space-y-4">
        ...
        <div className="pt-4">
          <Button className="flex w-full items-center justify-center gap-2 rounded-full text-base" variant={overallStatus.allComplete ? "default" : "outline"} onClick={handleSubmitWorkout} disabled={isSubmitting}>
            {overallStatus.allComplete ? <Check className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}
            <span>Complete &amp; Exit</span>
          </Button>
          {!overallStatus.allComplete && <p className="mt-2 text-center text-xs text-muted-foreground">
  ...
  const renderNavigation = () => {
    ...
    return <div className="fixed inset-x-0 bottom-0 border-t bg-background/95 pb-6 pt-4">
        <div className="mx-auto flex w-full max-w-md items-center justify-between px-6">
          <Button variant="outline" size="icon" className="rounded-full" disabled={isFirstExercise && !isReviewStep} onClick={handlePrev}>
          ...
          <Button variant={forwardIsPrimary ? "default" : "outline"} size="icon" className="rounded-full" disabled={isReviewStep} onClick={handleNext}>
  ...
  if (viewState.status === "error") {
    return <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-8">
